...
Command|Parameters
:---|:---
`HeatingZone<x>`| publishes mqtt info for `HeatingZone<x>`:<br>`HeatingZone1` -> `{"HeatingZone":{"mode":0,"label":"HTG1","expiry":1644737400,"info":"HTG1 Auto Off until 07:30 Sun 13 Feb 22","id":1,"power":false}}`<br><br>With power param:<br>`HeatingZone<x> 1`<br><br>where<br>`0/off/false` = turn OFF<br>`1/on/true` = turn ON<br><br>With heat/idle param:<br>`HeatingZone<x> heat`<br>`HeatingZone<x> idle`<br><br>where<br>`heat` = turn relay ON if zone is running<br>`idle` = turn relay OFF if zone is running<br><br>The `heat`/`idle` param is useful when separate thermostats need to send `heat`/`idle` commands to the heating controller. In this scenario the heating controller acts as the scheduler/timer and the separate thermostats call for heat by controlling the relays.<br><br>To update a zone's label or mode:<br>`HeatingZone<x> {"update": {"mode": 5, "label": "HTG1"}`<br><br>When mode = 1 (boost), specify hours:<br>`HeatingZone<x> {"update": {"mode": 1, "hours": 2}}`<br><br>To update the target or room temperature (together or individually):<br>`HeatingZone<x> {"update": {"target": 22.5, "room": 21.3}}`<br>`HeatingZone<x> {"update": {"target": 22}}`<br>`HeatingZone<x> {"update": {"room": 21}}`<br>NB: The THERM option needs to be enabled via the web UI or the HeatingOptions command. The `target` temperature key can optionally be included in the payload when creating a new zone.<br><br>To add a new zone:<br>`HeatingZone {"new": {"label": "2nd Flr", "mode": 1, "target": 22.5}}`<br><br>where mode :<br>`0` (Auto)<br>`1` (Boost)<br>`2` (Const On)<br>`3` (Const Off)<br>`4` (Adv)<br>`5` (All Day)<br><br>To delete a zone:<br>`HeatingZone<x> delete`<br><br>To show the currently running schedule for a given zone:<br>`HeatingZone<x> RunningSchedule`<br><br>Note that the inclusion of heat, idle, target or room params results in an additional HeatingDisplay#HeatingStatus payload being published when the DISPLAY option is enabled. This enables displays to be updated:<br>`{"HeatingDisplay":{"HeatingStatus":{"room temp":21,"zone":1,"target temp":22,"heat":true}}}`
...
`HeatingOptions` | When no json payload is specified the command returns the current settings for all options:<br>`{"HeatingOptions": {"UI": 1,"LED": 1,"DISPLAY": 1,"THERM": 1,"MQTT": 1}}`<br><br>`0` = OFF<br>`1` = ON<br><br>To set 1 or more options use a json payload and specify the option name as string and <br><br>`0/off/false` to turn option off<br>`1/on/true` to turn option on:<br><br>`HeatingOptions {"LED": 1, "Display": 1}`  
